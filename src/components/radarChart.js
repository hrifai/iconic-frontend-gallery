...
function RadarChart(id, data, options) {
  ...
  console.log(data);
  // data.forEach(arr => arr.forEach(e => (e.value = Math.round(e.value * 100) / 100)));

  var cfg = {
    w: 600, //Width of the circle
    h: 600, //Height of the circle
    margin: { top: 20, right: 20, bottom: 20, left: 20 }, //The margins of the SVG
    legendPosition: {x: 20, y: 555},
  ...
  axisGrid
    ...
    .attr("fill", "#ffffff")
    .text(function(d, i) {
      return Math.round((Format((maxValue * d) / cfg.levels)) * 100) / 100 ;
    });
  ...
  if(!(id == '.smallRadarChart')) {
    ...
    // .shape("path", d3.svg.symbol().type("triangle-up").size(150)())
      ...
      .labels(cfg.color.domain().map(function (d) {
        // console.log(data[d][0].areName);
        return data[d][0].areName;
      }))
